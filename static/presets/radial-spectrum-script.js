...
export async function startPreset(varsModule) {
  ...
  const fragmentShaderSource = `
    ...
    vec3 drawRing() {
      vec2 centeredUV = v_uv * 2.0 - 1.0;
      float radius = length(centeredUV);
      float angle = atan(centeredUV.y, centeredUV.x);
      float t = (angle + 3.14159265) / 6.2831853;
      float barIndex = floor(t * u_barCount);
      float amplitude = getAmplitude(barIndex);

      float targetRadius = ${visualizerVariables.base_radius ?? 0.35} +
                           ${visualizerVariables.radius_scale ?? 0.5} * amplitude;
      float ringBand = 1.0 - smoothstep(0.01, 0.0, abs(radius - targetRadius));
      ...
      return ringColor * ringBand;
    }

    void main() {
      vec3 ring = drawRing();
      gl_FragColor = vec4(pow(ring, vec3(${visualizerVariables.intensity_gamma ?? 1.0})), 1.0);
    }
  `;
